...
extern int slh_perf_fd;

static SLH_INLINE uint64_t slh_get_cycles(void) {
  uint64_t count;
  if (read(slh_perf_fd, &count, sizeof(uint64_t)) == -1) {
    return 0;
  }
  return count;
}
...
#include <x86intrin.h>
static SLH_INLINE uint64_t slh_get_cycles(void) { return __rdtsc(); }
#elif defined(__aarch64__)
static SLH_INLINE uint64_t slh_get_cycles(void) {
  uint64_t t;
